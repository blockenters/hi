...
SELECT  TIMEDIFF(  now() , order_date ) 
from orders;
...
SELECT  MONTH( order_date ) as order_month , count( * ) as cnt
from orders
...
SELECT *, DAYNAME(order_date), DAYOFWEEK(order_date), WEEKDAY(order_date) 
from orders;
...
-- 오늘 기준으로, 앞으로 3일 이내에 배송 예정인 주문들을 가져오시오.
SELECT *
from orders;
...
values
('홍길동', '의자', '2024-12-01 13:30:00', '2024-12-03');
